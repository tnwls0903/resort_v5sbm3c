...
-- CREATE
INSERT INTO drama(cateno, name, namesub, cnt, rdate, seqno, visible)
...
-- DELETE: 삭제
DELETE FROM drama WHERE cateno=3;
...
-- 관련 글수 증가
UPDATE drama SET cnt = cnt + 1 WHERE cateno = 1;
SELECT * FROM drama;
...
-- 출력 순서 높임: seqno 10 -> 1
UPDATE drama SET seqno = seqno - 1 WHERE cateno = 1;
...
-- 카테고리 공개 설정
UPDATE drama SET visible='Y' WHERE cateno = 1;
...
-- 카테고리 비공개 설정         
UPDATE drama SET visible='N' WHERE cateno = 1;
...
--    CATENO NAME                           NAMESUB                               CNT RDATE                    SEQNO V
------------ ------------------------------ ------------------------------ ---------- ------------------- ---------- -
--        20 오버렌딩                       -                                       0 2024-03-20 10:19:06          2 Y

-- 회원/비회원에게 공개할 소분류 목록
SELECT cateno, name, namesub, cnt, rdate, seqno, visible
FROM drama 
WHERE name = '오버렌딩' AND namesub != '-' AND visible = 'Y'
ORDER BY seqno ASC;
--    CATENO NAME                           NAMESUB                               CNT RDATE                    SEQNO V
------------ ------------------------------ ------------------------------ ---------- ------------------- ---------- -
--        23 오버렌딩                       장비                                    0 2024-03-20 10:20:16          1 Y
--        24 오버렌딩                       캠핑                                    0 2024-03-20 10:20:22          2 Y
