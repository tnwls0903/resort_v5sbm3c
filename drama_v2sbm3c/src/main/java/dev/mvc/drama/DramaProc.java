...
//@Component("dev.mvc.drama.DramaProc")
@Service("dev.mvc.drama.DramaProc")
public class DramaProc implements DramaProcInter {
  @Autowired
  private DramaDAOInter dramaDAO;
  
  public DramaProc() {
    System.out.println("-> DramaProc created.");  
  }
  ...
  public DramaVO read(int dramano) {
    DramaVO dramaVO = this.dramaDAO.read(dramano);
    return dramaVO;
  }

  @Override
  public int update(DramaVO dramaVO) {
    int cnt = this.dramaDAO.update(dramaVO);
    return cnt;
  }

  @Override
  public int delete(int dramano) {
    int cnt = this.dramaDAO.delete(dramano);
    return cnt;
  }

  @Override
  public int update_seqno_forward(int dramano) {
    int cnt = this.dramaDAO.update_seqno_forward(dramano);
    return cnt;
  }

  @Override
  public int update_seqno_backward(int dramano) {
    int cnt = this.dramaDAO.update_seqno_backward(dramano);
    return cnt;
  ...
  public int update_visible_y(int dramano) {
    int cnt = this.dramaDAO.update_visible_y(dramano);
    return cnt;
  }

  @Override
  public int update_visible_n(int dramano) {
    int cnt = this.dramaDAO.update_visible_n(dramano);
    return cnt;
  }

  @Override
  public ArrayList<DramaVO> list_all_nation_y() {
    ArrayList<DramaVO> list = this.dramaDAO.list_all_nation_y();
    return list;
  }

  @Override
  public ArrayList<DramaVO> list_all_genre_y(String nation) {
    ArrayList<DramaVO> list = this.list_all_genre_y(nation);
    return list;
  }
  ...
  public ArrayList<DramaVOMenu> menu() {
    // 중분류 목록을 저장할 객체 선언
    ArrayList<DramaVOMenu> menu = new ArrayList<DramaVOMenu>();
    ...
    for (DramaVO dramaVO : list) {
      DramaVOMenu dramaVOMenu = new DramaVOMenu(); // 하나의 중분류 메뉴 그룹
      
      dramaVOMenu.setNation(dramaVO.getNation()); // 중분류명
      
      // 중분류명을 이용하여 소분류 목록을 가져옴.
      ArrayList<DramaVO> list_genre = this.dramaDAO.list_all_genre_y(dramaVO.getNation());
      dramaVOMenu.setList_genre(list_genre);
      
      if (list_genre.size() > 0) { // 소분류가 있는 경우만 메뉴 등록
        menu.add(dramaVOMenu);
      }
  ...
  public ArrayList<DramaVO> list_search(String word) {
    ArrayList<DramaVO> list = this.dramaDAO.list_search(word);
    return list;
  }
  ...
  public ArrayList<DramaVO> list_search_paging(String word, int now_page, int record_per_page) {
    ...
    */
    ...
    ArrayList<DramaVO> list = this.dramaDAO.list_search_paging(map);
    return list;
  }
  
  @Override
  public int list_search_count(String word) {
    int cnt = this.dramaDAO.list_search_count(word);
    return cnt;
  }
