...
//@Component("dev.mvc.hotplace.HotplaceProc")
@Service("dev.mvc.hotplace.HotplaceProc")
public class HotplaceProc implements HotplaceProcInter {
  @Autowired
  private HotplaceDAOInter hotplaceDAO;
  
  public HotplaceProc() {
    System.out.println("-> HotplaceProc created.");  
  }
  
  @Override
  public int create(HotplaceVO hotplaceVO) {
    int cnt = this.hotplaceDAO.create(hotplaceVO);
    return cnt;
  }

  @Override
  public ArrayList<HotplaceVO> list_all() {
    ArrayList<HotplaceVO> list = this.hotplaceDAO.list_all();
    return list;
  }

  @Override
  public HotplaceVO read(int hotplaceno) {
    HotplaceVO hotplaceVO = this.hotplaceDAO.read(hotplaceno);
    return hotplaceVO;
  }

  @Override
  public int update(HotplaceVO hotplaceVO) {
    int cnt = this.hotplaceDAO.update(hotplaceVO);
    return cnt;
  }

  @Override
  public int delete(int hotplaceno) {
    int cnt = this.hotplaceDAO.delete(hotplaceno);
    return cnt;
  }

  @Override
  public int update_seqno_forward(int hotplaceno) {
    int cnt = this.hotplaceDAO.update_seqno_forward(hotplaceno);
    return cnt;
  }

  @Override
  public int update_seqno_backward(int hotplaceno) {
    int cnt = this.hotplaceDAO.update_seqno_backward(hotplaceno);
    return cnt;
  ...
  public int update_visible_y(int hotplaceno) {
    int cnt = this.hotplaceDAO.update_visible_y(hotplaceno);
    return cnt;
  }

  @Override
  public int update_visible_n(int hotplaceno) {
    int cnt = this.hotplaceDAO.update_visible_n(hotplaceno);
    return cnt;
  }

  @Override
  public ArrayList<HotplaceVO> list_all_a_y() {
    ArrayList<HotplaceVO> list = this.hotplaceDAO.list_all_a_y();
    return list;
  }

  @Override
  public ArrayList<HotplaceVO> list_all_b_y(String a) {
    ArrayList<HotplaceVO> list = this.list_all_b_y(a);
    return list;
  }
  ...
  public ArrayList<HotplaceVOMenu> menu() {
    // 중분류 목록을 저장할 객체 선언
    ArrayList<HotplaceVOMenu> menu = new ArrayList<HotplaceVOMenu>();
    
    // 중분류 목록 로딩
    ArrayList<HotplaceVO> list = this.hotplaceDAO.list_all_a_y();
    
    // 중분류 + 소분류 결합 
    for (HotplaceVO hotplaceVO : list) {
      HotplaceVOMenu hotplaceVOMenu = new HotplaceVOMenu(); // 하나의 중분류 메뉴 그룹
      
      hotplaceVOMenu.setA(hotplaceVO.getA()); // 중분류명
      
      // 중분류명을 이용하여 소분류 목록을 가져옴.
      ArrayList<HotplaceVO> list_b = this.hotplaceDAO.list_all_b_y(hotplaceVO.getA());
      hotplaceVOMenu.setList_b(list_b);
      
      if (list_b.size() > 0) { // 소분류가 있는 경우만 메뉴 등록
        menu.add(hotplaceVOMenu);
      }
  ...
  public ArrayList<HotplaceVO> list_search(String word) {
    ArrayList<HotplaceVO> list = this.hotplaceDAO.list_search(word);
    return list;
  }
  ...
  public ArrayList<HotplaceVO> list_search_paging(String word, int now_page, int record_per_page) {
    ...
    */
    ...
    ArrayList<HotplaceVO> list = this.hotplaceDAO.list_search_paging(map);
    return list;
  }
  
  @Override
  public int list_search_count(String word) {
    int cnt = this.hotplaceDAO.list_search_count(word);
    return cnt;
  }
