  ...
  public String create(HttpServletRequest request, 
    ...
    if (memberProc.isMemberAdmin(session)) { // 관리자로 로그인한경우
      ...
      if (cnt == 1) {
        ...
        // return "redirect:/contents/list_by_cateno?cateno=" + contentsVO.getCateno();  // /templates/contents/list_by_cateno.html
      } else {
...
//  
//    this.contentsProc.list_by_cateno_search_count(map);
//    
...
//
  ...
  public String list_by_cateno_search_paging(HttpSession session, Model model, 
    ...
    // 페이징 버튼 목록
    int search_count = this.contentsProc.list_by_cateno_search_count(map); // 갯수 가져오기
    String paging = this.contentsProc.pagingBox(cateno, 
                                                            now_page, 
                                                            word, 
                                                            "/contents/list_by_cateno", 
                                                            search_count, 
   ...
   * 목록 + 검색 + 페이징 지원 + Grid
   * 검색하지 않는 경우
   * http://localhost:9091/contents/list_by_cateno_grid.do?cateno=2&word=&now_page=1
   * 검색하는 경우
   * http://localhost:9091/contents/list_by_cateno_grid.do?cateno=2&word=탐험&now_page=1
   * 
   * @param cateno
   * @param word
   * @param now_page
  ...
  public String list_by_cateno_search_paging_grid(HttpSession session, Model model, 
    ...
    return "contents/list_by_cateno_search_paging_grid"; // /templates/contents/list_by_cateno_search_paging.html
  }
...
//    contentsVO.setContent(content);  
  ...
  /**
   * MAP 등록/수정/삭제 처리
   * http://localhost:9091/contents/map
   * @param contentsVO
   * @return
   */
  ...
  /**
   * Youtube 등록/수정/삭제 폼
   * http://localhost:9091/contents/youtube?contentsno=1
   * @return
   */
  ...
  /**
  * 수정 폼
  * http://localhost:9091/contents/update_text?contentsno=1
  * @return
  */
 ...
 /**
  * 파일 수정 폼
  * http://localhost:9091/contents/update_file?contentsno=1
  * @return
  */
 ...
 public String delete(HttpSession session, 
   ...
   if (memberProc.isMemberAdmin(session)) { // 관리자로 로그인한경우
     ...
     ra.addAttribute("url", "/member/login_cookie_need"); // /WEB-INF/views/admin/login_need.jsp
